...
| [**spesifikasi-proyek.md**](spesifikasi-proyek.md) | **Pengembang** dan orang yang urus integrasi (Sheets, hosting, CDN) | Spesifikasi teknis: stack, struktur kolom Google Sheets, format URL `?to=`, logika tipe undangan (akad / resepsi / keduanya), kontrak API RSVP dan komentar, alur admin–tamu–sistem, strategi aset (dev lokal, produksi CDN + fallback `public/`), performa, validasi, dan contoh rumus spreadsheet untuk kolom tautan. |
